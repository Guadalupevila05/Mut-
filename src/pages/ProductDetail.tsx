...
import { useProduct, startChat, supabase } from "../lib/supabase";
...
export const ProductDetail = () => {
  ...
  const handleContact = async () => {
    if (!seller) return;
    const chatId = await startChat(product.id, seller.id);
    if (chatId) navigate(`/chat/${chatId}`);
    else navigate("/auth");
  };

  const renderTitle = (title: string, style: string | null) => {
              ...
              <div className="p-5 rounded-[2rem] border border-ink/5 dark:border-white/5 bg-white/50 dark:bg-darkBg-alt/50 mb-8">
                ...
                {seller.neighborhood && (
                  <div className="flex items-center gap-2 text-[11px] font-medium text-ink/60 bg-softGray dark:bg-darkBg rounded-xl px-3 py-2">
                    <MapPinIcon size={12} />
                    <span>Envío o retiro en {seller.neighborhood}, Tandil</span>
                  </div>
                )}
              </div>
            )}
          ...
          {!isOwnProduct ? (
            <div className="sticky bottom-24 md:bottom-0 mt-auto pt-4 bg-warmWhite/90 dark:bg-darkBg/90 backdrop-blur-xl md:bg-transparent md:backdrop-blur-none flex flex-col gap-3 z-30 border-t border-ink/5 md:border-none -mx-4 px-4 md:mx-0 md:px-0 pb-4 md:pb-0">
              ...
              >
                ...
                {(seller?.name ?? seller?.username ?? "").split(" ")[0]}
              </Button>
            </div>
          ) : (
            <div className="sticky bottom-24 md:bottom-0 mt-auto pt-4 backdrop-blur-xl md:bg-transparent md:backdrop-blur-none z-30 border-t border-ink/5 md:border-none -mx-4 px-4 md:mx-0 md:px-0 pb-4 md:pb-0">
              <p className="text-center text-xs uppercase tracking-widest font-semibold text-ink/40 dark:text-warmWhite/40 py-3">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="relative mb-8">
                ...
                <input
                  type="number"
                  defaultValue={Math.round(product.price * 0.8)}
